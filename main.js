...
let goThroughModel = function(){
  ...
  while(lastNotes.length > 2){
    ...
    let lastTimesTensor = tf.reshape(tf.tensor2d([lastNotes.slice(0,3)], [1, 3], 'int32'), [1,3,1]);
  ...
  let probs = Array.from(prediction[1].slice([0,0,0],[1,4,1]).dataSync());
...
let playLoop = async function(){
	...
	while(play){
		...
		currentTone = pred();
		timeLeft = (currentTime * 200) - (performance.now() - timeLeft);
		if (timeLeft > 0){
